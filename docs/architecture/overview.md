...
|---|---|---|---|
| **Android App** | External (`Niccher/MPesa-Analyzer-App`) | Kotlin, Android SDK 35, Jetpack Compose | Captures local SMS, performs regex pre-classification, encrypts with dynamic IV AES-128-CBC, and streams uploads. |
| **Web & API Gateway** | `web/` | PHP 8.3, Apache, CodeIgniter 4, Shield | Decrypts payloads, persists raw SMS in `tbl_Sms`, exposes dashboard UI, manages user accounts and budgets. |
| **ML Intelligence** | `ml/` | Python 3.12, FastAPI, llama-server, Qwen2.5 1.5B | Autonomous background polling, known-sender dictionary lookup, LLM sender classification, and batch transaction extraction. |
| **Database** | Root `docker-compose.yml` | MySQL 8.4 (InnoDB) | Central persistence for raw SMS, classification profiles, transactions, and audit jobs. |
